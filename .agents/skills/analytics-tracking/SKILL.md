...
### dataLayer実装
- 設計した計測要件に基づき、アプリケーション側で必要な `dataLayer.push` コードを提示
- イベント名・パラメータ名・型・値の仕様を一覧表で明示
- ECサイトの場合は ecommerce オブジェクトの構造（items 配列のスキーマ）も提示
...
| スキル | 連携内容 |
|--------|----------|
| **google-ads-strategy** | 設計したキャンペーンのコンバージョンタグ・拡張コンバージョンをこのスキルで実装する |
| **meta-ads-strategy** | 設計したキャンペーンのMeta Pixel・Conversions APIをこのスキルで実装する |
| **tiktok-ads-strategy** | 設計したキャンペーンのTikTok Pixel・Events APIをこのスキルで実装する |
| **x-ads-strategy** | 設計したキャンペーンのX Pixel・Conversion APIをこのスキルで実装する |
| **cro** | CRO施策（ポップアップ表示・フォーム改善等）の効果計測イベントを設計する |
| **seo-analysis** | PSIで計測したCWVの継続モニタリングをGA4イベントで補完する |

広告戦略スキルでキャンペーンを設計した後にこのスキルで計測を実装する流れが典型的。先に広告戦略側でCV定義（プライマリCV/セカンダリCV）が決まっていると、計測設計がスムーズに進む。
