---
name: slide-deck
description: React + Tailwind でプレゼン資料（スライドデッキ）を作成し、Playwright + pdf-lib で PDF に書き出すスキル。ユーザーがスライド・プレゼン資料・提案書・ピッチデッキの作成に言及した場合に使用する。
---
...
│   ├── App.tsx                    # プレビュー＋エクスポート切替
│   ├── Slide.tsx                  # スライド外枠コンポーネント
│   └── export-slides.ts          # PNG→PDF書き出しスクリプト
...
スライドは 1280x720px（16:9）。デザインの詳細は [references/design-guide.md](references/design-guide.md) を参照すること。特にセクション6「リッチデザイン」はスライド品質に直結するため、初回作成時に必ず読む。
